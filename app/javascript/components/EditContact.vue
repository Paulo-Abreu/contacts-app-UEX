...
export default {
  ...
  methods: {
    loadContact() {
      ...
      axios.get(`/contacts/${this.contactId}.json`)
        .then(response => {
          this.contact = response.data;

          // Extrair o código do país do telefone e combinar com o país
          const country = this.countries.find(c => this.contact.phone.startsWith(c.code));
          this.contact.country = country || { name: 'Brazil', code: '+55' };

          this.updatePhoneMask();
        })
        .catch(error => {
          console.error('Erro ao carregar o contato:', error.response ? error.response.data : error.message);
        });
    },
    updateContact() {
      // Atualizar o contato
      ...
      axios.put(`/contacts/${this.contactId}`, { contact: this.contact })
        .then(response => {
          console.log('Contato atualizado com sucesso:', response);
          window.location = '/';
        })
        .catch(error => {
          console.error('Erro ao atualizar o contato:', error.response ? error.response.data : error.message);
        });
    },
    fetchAddress() {
